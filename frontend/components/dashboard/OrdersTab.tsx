...
const OrdersTab: React.FC<OrdersTabProps> = ({ onRefresh }) => {
  ...
  const renderOrderItem = useCallback(({ item: order }: { item: Order }) => {
    ...
    return (
      <View className="bg-white rounded-2xl p-5 mb-4 shadow-lg border border-gray-100 mx-4">
        {/* Order Header */}
        ...
        <View className="space-y-3">
            ...
            <View className="bg-gray-50 rounded-xl p-3">
              ...
              {order.products.slice(0, 2).map((item, index) => (
                <View key={index} className="flex-row justify-between items-center py-1">
                  <Text className="text-sm text-gray-600 flex-1 font-medium">
                    {item.product.name}
                  </Text>
          ...
          {/* Order Summary */}
          <View className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-4">
            <View className="flex-row justify-between items-center mb-2">
              ...
              <View className="flex-row items-center">
                ...
                />
                <Text 
                  className={`text-sm ml-1 font-semibold ${order.isPaid ? 'text-green-600' : 'text-yellow-600'}`}
                >
  ...
  const renderEmptyState = useCallback(() => (
    <View className="flex-1 justify-center items-center py-16 px-8">
      <View className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-full p-8 mb-6">
        <FontAwesome name="shopping-cart" size={48} color="#3B82F6" />
      </View>
      ...
      <View className="py-6">
        ...
        >
          {loadingMore ? (
            ...
            <View className="flex-row items-center">
              <FontAwesome name="chevron-down" size={16} color="#3B82F6" />
  ...
  if (loading && !refreshing) {
    return (
      <View className="flex-1 justify-center items-center">
        <View className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-full p-6 mb-4">
...
const OrderDetailsContent: React.FC<{ order: Order }> = ({ order }) => {
  ...
  return (
    <View className="space-y-4">
      {/* Order Status */}
      <View className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-4 border border-blue-100">
        <View className="flex-row justify-between items-center mb-3">
          <View>
            <Text className="text-sm text-gray-500 font-medium">Order ID</Text>
            <Text className="text-lg font-bold text-gray-900">#{orderId}</Text>
          </View>
          <View 
        ...
        <View className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
          ...
          <View className="space-y-3">
            {order.products.map((item, index) => (
              <View key={index} className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl p-4 border border-gray-100">
                <View className="flex-row justify-between items-start mb-3">
                  <Text className="text-base font-bold text-gray-900 flex-1 mr-3">
                    {item.product.name}
                  </Text>
      ...
      {/* Order Summary */}
      <View className="bg-gradient-to-r from-green-50 to-blue-50 rounded-xl p-4 border border-green-100 shadow-sm">
        <View className="flex-row items-center mb-4">
      ...
      <View className="space-y-3 pt-2">
        <TouchableOpacity
          onPress={handleAddMoreProducts}
          className="bg-gradient-to-r from-blue-600 to-purple-600 py-4 rounded-xl flex-row justify-center items-center shadow-lg"
        >
